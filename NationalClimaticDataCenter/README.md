...
and weather reporting authorities. Much of these inconsistenties can be tolerated 
if the Storm Events Database were examined in very small time frames for limited 
geographic areas, but to report on the database in the aggregate we need to smooth 
out these inconsistenties by substitution – that is, use a concordance and/or lookup 
table to transform data entries into consistent and understandable terms. For 
example, one of the most egregious data reporting practices was the use of the 
"begin county name" field to annotate a storm track that covered multiple counties 
within a state. The "begin county fips" field would be encoded with something other 
than the actual county fips designation (often "000" or a "Public Forecast Zone" 
designator) and the "begin county name" field would contain entries like: 
_WYZ001>006 - 008>015 - 017 - 022>025 - 050>070_ (which is a series of "Public 
Forecast Zone" designations, typically for a particular "Weather Forecast Office") 
which, in this example, reported a winter storm on November 21, 1995 in the state 
of Wyoming which started in "zone" 001, moved into and occupied "zones" 001 through 
006, 008 through 015, 017, 022 through 025, and 050 through 070. To determine what 
counties were involved requires the use of a concordance table to align the two 
reporting systems. This field can also contain county names, organized in the same 
fashion. It can also indicate storm events that span multiple time zones. Much of 
the sorting out of this plethoria of information can be done with the positioning 
coordinates (latitude and longitude) in the concordance tables that give the centroid 
of the zones and sometimes the centroid of the storm system. Together with size 
information (storm tract length, width, and direction) we could calculate the 
counties involved, the storm track, or visualize the resulting information on a 
map. Would could also discover key geographic details, such as the location or 
proximity of urban, industrial, and agricultural areas.
